...
void ServerProxy::requestDisconnect(const char *msg)
{
  m_events->addEvent(Event(
      EventTypes::ClientDisconnectRequested, m_stream->getEventTarget(),
      new Client::DisconnectRequest(Client::DisconnectRequest::Kind::Disconnect, msg)
  ));
}

void ServerProxy::requestRefuseConnection(deskflow::core::ConnectionRefusal reason, const char *msg)
{
  m_events->addEvent(Event(
      EventTypes::ClientDisconnectRequested, m_stream->getEventTarget(), new Client::DisconnectRequest(reason, msg)
  ));
}
