...
bool EventQueue::dispatchEvent(const Event &event)
{
  ...
  if (auto typeHandler = getHandler(event.getType(), target)) {
    typeHandler(event);
    return true;
  }
  if (auto anyHandler = getHandler(EventTypes::Unknown, target)) {
    anyHandler(event);
    return true;
  }
  return false;
...
EventQueue::EventHandler EventQueue::getHandler(EventTypes type, void *target) const
{
  std::scoped_lock lock{m_mutex};
