...
macOS users who download directly from releases may need to remove downloaded-file metadata after copying the app to the `Applications` dir:

```
xattr -dr com.apple.quarantine "/Applications/Deskflow.app"
xattr -d com.apple.provenance "/Applications/Deskflow.app" 2>/dev/null || true
```
